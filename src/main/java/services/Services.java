...
public class Services {
	...
	@Path("{title}")
	@Produces({ MediaType.APPLICATION_JSON })
	public Response searchSong(@PathParam("title") String title) {
		SearchSongCommand search = new SearchSongCommand();
		String songString = null;
		try {
			songString = mapper.writeValueAsString(search.execute(title));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return Response.status(200).entity(songString).build();
